...
// Добавьте функцию проверки файла в начало файла
function isFileLike(obj: unknown): obj is { buffer: Buffer; [key: string]: any } {
    return (
        obj !== null &&
        typeof obj === 'object' &&
        'buffer' in obj &&
        ...
        async (c) => {
            ...
            if (isFileLike(image)) {
                ...
                const file = await storage.createFile(
                    IMAGES_BUCKET_ID,
                    ID.unique(),
                    image // тут должен быть объект с buffer и т.п.
                )
    ...
    .get('/', sessionMiddleware, zValidator('query', z.object({workspaceId: z.string()})),
        ...
        async (c) => {
            ...
            if (isFileLike(image)) {
                ...
                uploadedImageUrl = `https://cloud.appwrite.io/v1/storage/buckets/${IMAGES_BUCKET_ID}/files/${file.$id}/preview?project=${process.env.NEXT_PUBLIC_APPWRITE_PROJECT}`
